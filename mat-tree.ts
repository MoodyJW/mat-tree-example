...
@Component({
  selector: 'ui-max-mt-admin-user-details',
...
export class AdminUserDetailsComponent {
   ...
   */
  transformer = (node: TreeItemNode, level: number): TreeItemFlatNode => {
  ...
  permissionsFilterChanged(searchValue: string): void {
    this.database.filter(searchValue);
    if (searchValue) {
